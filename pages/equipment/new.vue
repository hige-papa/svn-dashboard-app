  ...
  <div class="page-container">
    <div class="container">
      <div class="header">
        ...
        <p class="page-subtitle">{{ isEditMode ? '備品編集' : '新規備品登録' }}</p>
      </div>
        ...
        <form @submit.prevent="handleSubmit" class="form-grid">
          <div class="form-section">
            ...
            <div class="form-group">
              ...
              <textarea 
                ...
                class="form-textarea" 
                placeholder="備品の詳細な説明や注意事項などを入力..."
                rows="4"
              ></textarea>
            </div>
          </div>
          ...
          <div class="form-actions">
            ...
            <button type="submit" :disabled="isLoading" class="btn btn-primary">
              ...
              {{ isLoading ? '保存中...' : (isEditMode ? '備品を更新' : '備品を作成') }}
            </button>
          </div>
        </form>
      </div>
...
const { getAsync, addAsync, updateAsync } = useEquipment()

// SEOメタタグ設定
useHead({
  title: 'TASCAL - 備品新規登録',
  meta: [
    { name: 'description', content: 'TASCALシステムで備品を新規登録できます' }
  ],
  link: [
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      ...
      showNotification('備品情報を更新しました！', 'success')
    } else {
      await addAsync(dataToSave)
      showNotification('備品を作成しました！', 'success')
    }
