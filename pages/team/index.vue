        ...
        <div class="stats-section">
          <div class="stat-card">
            ...
            <div class="stat-content">
              ...
              <div class="stat-label">総チーム種類</div>
            </div>
          </div>
...
const { getListAsync, deleteAsync } = useTeam()

// SEOメタタグ設定
useHead({
  title: 'TASCAL - チーム管理',
  meta: [
    { name: 'description', content: 'TASCALシステムのチーム管理画面です' }
  ],
  link: [
...
const filteredItems = ref<Team[]>([])
const searchQuery = ref('')
const viewMode = ref<'grid' | 'list'>('grid')
const currentPage = ref(1)
const itemsPerPage = 12
const showDeleteModal = ref(false)
const selectedItem = ref<Team | null>(null)
...
const fetchAllItems = async (): Promise<Team[]> => {
...
const deleteItemApi = async (itemId: string): Promise<void> => {
  ...
  return await deleteAsync(itemId).then(_ => {
    teams.value.splice(teams.value.findIndex(e => { return e.id === itemId }), 1)
  })
}
// ---ここまで---
...
const loadItems = async () => {
  try {
    ...
    showNotification('チームの読み込みに失敗しました', 'error')
  }
}
...
const viewItemDetail = (item: Team) => {
  push(`/team/${item.id}`)
}

const editItem = (item: Team) => {
  push(`/team/${item.id}/edit`)
}

const deleteItem = (item: Team) => {
