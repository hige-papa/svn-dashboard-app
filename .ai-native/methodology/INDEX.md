...
| バージョン | 日付 | 変更概要 |
|-----------|------|---------|
| 1.12.0 | 2026-04-10 | 3対象者向けリリースノート自動生成機能追加。technical-writer.md D-5にリリースノート生成責務追加、RELEASE_NOTES_FORMAT（開発者向け・運用者向け・エンドユーザー向けの3テンプレート）新設。coding-agent.md OUTPUT_FORMATに変更分類（リリースノート向け）追加。phase-definitions.md Phase 7/8ゲート条件にリリースノート作成追加、出力パス規約更新（release-note.md → release-notes/ ディレクトリ）。phase-gate-checklists.md Phase 7/8チェックリスト更新。pm-scrum-master.md INTERACTION_RULES更新。全ドキュメントのフロントマターバージョンをv1.12.0に統一 |
