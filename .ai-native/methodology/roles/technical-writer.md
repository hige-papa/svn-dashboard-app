...
### D-5: リリースノート生成（Phase 7-8）

Phase 7（MVP）および Phase 8（フルリリース）のゲート判定時に、3対象者向けリリースノートを生成する。

- **入力ソース:**
  - コーディングエージェントの実装完了報告（変更分類フィールド）
  - `outputs/phaseN/dev-log.md`（開発ログ）
  - Git コミット履歴・PR 記述
  - `outputs/phase3/functional-requirements.md`（機能要件 → ユーザー向け表現へのマッピング）
- **出力:** `outputs/phase7/release-notes/` または `outputs/phase8/release-notes/` に3ファイル（RELEASE_NOTES_FORMAT 参照）
- **Phase 7:** MVP で実装された機能に限定したリリースノート
- **Phase 8:** 全機能を対象とした累積リリースノート
- **品質基準:**
  - 開発者向け: 破壊的変更が漏れなく記載されている
  - 運用者向け: デプロイ・ロールバック手順が具体的である
  - ユーザー向け: 技術用語を使わず、ユーザーの言葉で記述されている
...
## RELEASE_NOTES_FORMAT（リリースノート形式）

3対象者向けに、それぞれ独立したリリースノートファイルを生成する。配置先は `outputs/phase7/release-notes/`（MVP）または `outputs/phase8/release-notes/`（フルリリース）。
...
### release-notes-dev.md（開発者向け）

```markdown
# リリースノート（開発者向け）— vX.Y.Z

## 破壊的変更
（API変更、DB スキーマ変更、設定キー変更など。なければ「なし」）
...
## 依存関係の変更
（追加・更新・削除されたライブラリ。なければ「なし」）

## 環境変数・設定の変更
（新規・変更・廃止された設定項目。なければ「なし」）

## マイグレーション手順
（DB マイグレーション、データ移行手順。なければ「なし」）
```

### release-notes-ops.md（運用者向け）

```markdown
# リリースノート（運用者向け）— vX.Y.Z

## デプロイ時の注意事項
（デプロイ前に必要な作業、デプロイ順序の制約。なければ「なし」）
...
## 監視・アラートの変更
（新規メトリクス、アラート閾値変更、ログフォーマット変更。なければ「なし」）

## ロールバック手順
（問題発生時の切り戻し手順）

## サポート対応への影響
（問い合わせ増加が見込まれる変更、FAQ更新の要否）
```

### release-notes-user.md（エンドユーザー向け）

```markdown
# リリースノート — vX.Y.Z

## 新しくできるようになったこと
（機能名と、ユーザーにとっての価値を平易な言葉で）

## 改善されたこと
（使いやすさ・速度・表示等の改善を、ユーザーが体感する言葉で）

## 修正されたこと
（ユーザーが遭遇していた問題とその解決を、技術用語なしで）
...
| コーディングエージェント | 実装の振る舞いの詳細（エラーメッセージの文言、画面遷移のフロー等）を確認する。実装完了報告の変更分類をリリースノート生成の入力として受け取る |
...
**アーティファクトパス:**
- **出力:** `outputs/knowledge-base/` 配下のMDファイル、`outputs/knowledge-base/_index.md`、`outputs/phaseN/release-notes/`（3対象者向けリリースノート）
