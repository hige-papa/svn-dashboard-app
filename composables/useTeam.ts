...
export const useTeam = () => {
    ...
    const { teamDocRoot } = useDocumentRoot();
    ...
    const addAsync = async (m: any) => {
        ...
        const ref = getDocRef(teamDocRoot.document(id))
    ...
    const getListAsync = async (...qc: QueryConstraint[]) => {
        const result = [] as any[]
        await getCollectionAsync(teamDocRoot.collection(), ...qc).then(async response => {
            if (!response.empty) {
    ...
    const countAsync = async (...qc: QueryConstraint[]) => {
        return await countCollectionAsync(teamDocRoot.collection(), ...qc).catch(_ => {
            return 0;
        });
    };
    ...
    const loadAsync = async (take: number = 20) => {
        ...
        const response = await getCollectionAsync(teamDocRoot.collection(), ...query);
