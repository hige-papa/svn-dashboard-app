...
export const useEquipment = () => {
    ...
    const { equipmentDocRoot } = useDocumentRoot();
    ...
    const addAsync = async (m: any) => {
        ...
        const ref = getDocRef(equipmentDocRoot.document(id))
    ...
    const getListAsync = async (...qc: QueryConstraint[]) => {
        const result = [] as any[]
        await getCollectionAsync(equipmentDocRoot.collection(), ...qc).then(async response => {
            if (!response.empty) {
    ...
    const countAsync = async (...qc: QueryConstraint[]) => {
        return await countCollectionAsync(equipmentDocRoot.collection(), ...qc).catch(_ => {
            return 0;
        });
    };
    ...
    const loadAsync = async (take: number = 20) => {
        ...
        const response = await getCollectionAsync(equipmentDocRoot.collection(), ...query);
