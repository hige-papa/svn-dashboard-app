...
export const useSection = () => {
    ...
    const { sectionDocRoot } = useDocumentRoot();
    ...
    const addAsync = async (m: any) => {
        ...
        const ref = getDocRef(sectionDocRoot.document(id))
    ...
    const getListAsync = async (...qc: QueryConstraint[]) => {
        const result = [] as any[]
        await getCollectionAsync(sectionDocRoot.collection(), ...qc).then(async response => {
            if (!response.empty) {
    ...
    const countAsync = async (...qc: QueryConstraint[]) => {
        return await countCollectionAsync(sectionDocRoot.collection(), ...qc).catch(_ => {
            return 0;
        });
    };
    ...
    const loadAsync = async (take: number = 20) => {
        ...
        const response = await getCollectionAsync(sectionDocRoot.collection(), ...query);
